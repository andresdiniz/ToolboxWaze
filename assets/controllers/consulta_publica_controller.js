...
export default class extends Controller {
    ...
    connect() {
        ...
        this.listTarget.addEventListener('click', (event) => {
            const button = event.target.closest('[data-share-item]');
            if (!button) return;
    ...
    card(item) { const title = item.nome || item.nomeFantasia || item.escola || item.razaoSocial || item.tipo || 'Registro público'; const fields = Object.entries(item).filter(([key, value]) => !['id', 'linkWaze'].includes(key) && value !== null && value !== undefined && String(value).trim() !== '').slice(0, 28); const summary = fields.slice(0, 4).map(([key, value]) => `${this.humanize(key)}: ${value}`).join(' | '); return `<article class="public-result-card"><div class="public-result-card__head"><h3>${this.escape(title)}</h3><button type="button" class="public-button public-button--soft public-share-item" data-share-item data-share-title="${this.escape(title)}" data-share-text="${this.escape(summary)}">Compartilhar este registro</button></div><div class="public-detail-grid">${fields.map(([key, value]) => `<div class="public-detail-item"><span>${this.escape(this.humanize(key))}</span><strong>${this.escape(value)}</strong></div>`).join('')}</div></article>`; }

    async shareAll() { await this.share(window.location.href, document.title, `Consulta pública de ${this.typeLabel()}`); }
    async shareItem(button) { const title = button.dataset.shareTitle || 'Registro público'; const text = button.dataset.shareText || title; const url = new URL(window.location.href); url.searchParams.set('tipo', this.type); url.searchParams.set('uf', this.stateTarget.value); url.searchParams.set('municipio', this.cityTarget.value); if (this.queryTarget.value.trim()) url.searchParams.set('q', this.queryTarget.value.trim()); await this.share(url.toString(), title, text); }
    async share(url, title, text = '') { try { if (navigator.share) await navigator.share({ title, text, url }); else { await navigator.clipboard.writeText(`${text ? `${text}\n` : ''}${url}`); this.showHelper('Link copiado para a área de transferência.'); } } catch {} }
    ...
    showHelper(text) { this.updateHelper(text); }
