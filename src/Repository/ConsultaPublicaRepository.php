...
final class ConsultaPublicaRepository extends ServiceEntityRepository
{
    ...
    {
        foreach (['uf', 'municipio'] as $field) if (!empty($filters[$field])) $qb->andWhere(sprintf('UPPER(%s.%s) = UPPER(:%s)', $alias, $field, $field))->setParameter($field, trim((string) $filters[$field]));
        if (!empty($filters['q'])) {
            if ($tipo === 'radar') { $qb->leftJoin($alias . '.faixas', 'faixaBusca'); $expressions = [sprintf('LOWER(%s.municipio) LIKE LOWER(:q)', $alias), sprintf('LOWER(%s.logradouro) LIKE LOWER(:q)', $alias), sprintf('LOWER(%s.numeroSerie) LIKE LOWER(:q)', $alias), 'LOWER(faixaBusca.numeroInmetro) LIKE LOWER(:q)']; }
            elseif ($tipo === 'posto') { $expressions = [sprintf('LOWER(%s.razaoSocial) LIKE LOWER(:q)', $alias), sprintf('LOWER(%s.nomeFantasia) LIKE LOWER(:q)', $alias), sprintf('LOWER(%s.municipio) LIKE LOWER(:q)', $alias), sprintf('LOWER(%s.bandeira) LIKE LOWER(:q)', $alias)]; }
            else { $expressions = [sprintf('LOWER(%s.municipio) LIKE LOWER(:q)', $alias)]; }
